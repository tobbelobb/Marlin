...
void report_xyz_from_stepper_position() {
  ...
  SERIAL_PROTOCOLPGM(", ");
  SERIAL_PROTOCOL(cartes[Y_AXIS]);
  SERIAL_PROTOCOLPGM(", ");
  SERIAL_PROTOCOL(cartes[Z_AXIS]);
  SERIAL_CHAR(']');
  SERIAL_EOL();
}
...
inline void gcode_M114() {
  ...
  planner.synchronize();
    ...
    if (parser.ushortval('S') == 1) {
      report_axis_position_from_encoder_data();
      return;
    }
  #endif

  if (parser.ushortval('S') == 2) {
    report_xyz_from_stepper_position();
    return;
  }
    ...
    if (report) LOOP_NUM_AXIS(i) switch (i) {
      ...
      case Z_AXIS:
        ...
        case D_AXIS:
          // D is connected on the first of E1, E2, E3, E4 output that is not an extruder
          #if E1_IS_TRINAMIC && EXTRUDERS == 1
            TMC_SAY_CURRENT(E1); break;
          #endif
          #if E2_IS_TRINAMIC && EXTRUDERS == 2
            TMC_SAY_CURRENT(E2); break;
          #endif
          #if E3_IS_TRINAMIC && EXTRUDERS == 3
            TMC_SAY_CURRENT(E3); break;
          #endif
          #if E4_IS_TRINAMIC && EXTRUDERS == 4
            TMC_SAY_CURRENT(E4); break;
          #endif
      #endif
      case E_AXIS:
        #if E0_IS_TRINAMIC
...
void ok_to_send() {
  ...
  void forward_kinematics_HANGPRINTER(float a, float b, float c, float d){
    float Asq =                  sq(anchor_A_y) + sq(anchor_A_z),
          Bsq = sq(anchor_B_x) + sq(anchor_B_y) + sq(anchor_B_z),
          Csq = sq(anchor_C_x) + sq(anchor_C_y) + sq(anchor_C_z),
          Dsq =                                   sq(anchor_D_z),
          aa = a*a,
          dd = d*d;
    float k0b = (-b*b + Bsq - Dsq + dd)/(2.0*anchor_B_x) + (anchor_B_y/(2.0*anchor_A_y*anchor_B_x))*(Dsq - Asq + aa - dd),
          k0c = (-c*c + Csq - Dsq + dd)/(2.0*anchor_C_x) + (anchor_C_y/(2.0*anchor_A_y*anchor_C_x))*(Dsq - Asq + aa - dd);
    float k1b = (anchor_B_y*(anchor_A_z - anchor_D_z))/(anchor_A_y*anchor_B_x) + (anchor_D_z - anchor_B_z)/anchor_B_x,
  ...
  inline bool prepare_kinematic_move_to(const float (&rtarget)[XYZE]) {
    ...
    while (--segments) {
      ...
      ADJUST_DELTA(raw); // Adjust Z if bed leveling is enabled
        ...
        // i.e., Complete the angular vector in the given time.
        if (!planner.buffer_segment(delta[A_AXIS], delta[B_AXIS], raw[Z_AXIS], raw[E_CART], HYPOT(delta[A_AXIS] - oldA, delta[B_AXIS] - oldB) * inverse_secs, active_extruder))
          break;
        /*
    ...
    // Ensure last segment arrives at target location.
      ...
      if (diff2) {
        planner.buffer_segment(delta[A_AXIS], delta[B_AXIS], rtarget[Z_AXIS], rtarget[E_CART], SQRT(diff2) * inverse_secs, active_extruder);
        /*
  ...
  void plan_arc(
    ...
    for (uint16_t i = 1; i < segments; i++) { // Iterate (segments-1) times
      ...
      clamp_to_software_endstops(raw);
        ...
        if (!planner.buffer_segment(pos[X_AXIS], pos[Y_AXIS], pos[Z_AXIS], raw[E_CART], fr_mm_s, active_extruder))
          break;
      #else
    ...
    // Ensure last segment arrives at target location.
      ...
      const float diff2 = HYPOT2(delta[A_AXIS] - oldA, delta[B_AXIS] - oldB);
      if (diff2)
        planner.buffer_segment(delta[A_AXIS], delta[B_AXIS], cart[Z_AXIS], cart[E_CART], SQRT(diff2) * inverse_secs, active_extruder);
    #elif HAS_UBL_AND_CURVES
      float pos[XYZ] = { cart[X_AXIS], cart[Y_AXIS], cart[Z_AXIS] };
      planner.apply_leveling(pos);
      planner.buffer_segment(pos[X_AXIS], pos[Y_AXIS], pos[Z_AXIS], cart[E_CART], fr_mm_s, active_extruder);
    #else
